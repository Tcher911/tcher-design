...
// can offer `npx tcher skills update`. Everything here is best-effort and
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    ...
    `Update now? It runs \`npx tcher skills update\`." ` +
    `If they agree, run \`npx tcher skills update\` (the update applies to the next session, not this one). ` +
