...
//   New installs write project config at .tcher/live/config.json instead.
...
/**
 * Read and parse the tcher skill source.
 ...
 * through to a compiled harness dir (`.agents/skills/tcher`) instead.
 */
 ...
 *
 * Defaults to the main tcher SKILL.md but accepts any relative path so
 ...
 *
 * The tcher-evals registry at `tools/instruction-rules.ts` pins
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  'tcher',               // foundational skill, not a steering command
  'teach-tcher',         // deprecated shim
...
export function replacePlaceholders(content, provider, commandNames = [], allSkillNames = []) {
  ...
  // After the v3.0 consolidation, commands are sub-commands of /tcher.
  // If there's only one user-invocable skill (tcher), generate sub-command references.
  ...
  if (nonExcluded.length === 0) {
    // Single-skill architecture: list sub-commands as /tcher <sub>
    commandList = TCHER_SUB_COMMANDS
      .map(n => `${cmdPrefix}tcher ${n}`)
      .join(', ');
  } else {
