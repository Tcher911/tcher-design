...
/**
 * Generate authoritative counts from source data and validate that key
 * files reference the correct numbers.
 */
function generateCounts(rootDir, skills) {
  // Count active commands. After the v3.0 consolidation, commands are sub-commands
  // of /tcher. Count them from the command router table in SKILL.md.
  const tcherSkill = skills.find(s => s.name === 'tcher');
  let commandCount;
  if (tcherSkill) {
    ...
    const routerMatches = tcherSkill.body.match(/^\| `[^`]+` \|/gm);
...
/**
 * Narrow prose check for the tcher skill source.
 *
...
async function build() {
  ...
  const deprecatedLocalSkills = [
    'frontend-design', 'teach-tcher',
    'arrange', 'normalize', 'onboard', 'extract',
    // v3.0 consolidation: standalone skills -> /tcher sub-commands
  ...
  // dirs above stay where they are because `npx skills add Tcher911/tcher-design`
  ...
  const claudeSkillsSrc = path.join(DIST_DIR, 'claude-code', '.claude', 'skills', 'tcher');
  if (fs.existsSync(claudeSkillsSrc)) {
    fs.mkdirSync(pluginSkillsDir, { recursive: true });
    copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, 'tcher'));
  }
  ...
  if (countErrors > 0 || proseErrors > 0 || skillProseErrors > 0) {
    process.exit(1);
  }
