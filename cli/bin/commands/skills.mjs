/**
 * `tcher skills` subcommand
 *
 * Usage:
 *   tcher skills help      Show all available skills and commands
 *   tcher skills install   Install compiled skills from the universal bundle
 *   tcher skills link      Symlink compiled skills from a local checkout
 *   tcher skills update    Update skills to latest version
 */
...
/**
 * Read the skills version from the tcher SKILL.md frontmatter.
 */
function getSkillsVersion(root) {
  for (const d of PROVIDER_DIRS) {
    const skillMd = join(root, d, 'skills', 'tcher', 'SKILL.md');
 ...
 * skills that aren't part of tcher). Deduplicates providers that
...
function isAlreadyInstalled(root, providers = PROVIDER_DIRS) {
  for (const d of providers) {
    ...
    try {
      ...
      // Look for 'tcher' skill (or prefixed variant, or legacy 'teach-tcher')
      if (entries.some(e =>
        e === 'tcher' || e.endsWith('-tcher') ||
        e === 'teach-tcher' || e.endsWith('-teach-tcher')
      )) {
        return d;
      }
    } catch {}
 ...
 * prefix (default `i-`), renaming the skill to e.g. `i-tcher`. The prefix
 * only earned its keep when every command was its own skill; with a single
 * `tcher` skill it does nothing, so it is no longer offered. Rename any
 * prefixed tcher skill back to the canonical `tcher` (the fresh
 * install/update content lands there next) so users aren't left with a stale,
 * orphaned `i-tcher` alongside the new one. Scoped to the tcher skill
 * by name -- never touches third-party skills that happen to start with `i-`.
 * Returns the number of skills migrated.
 */
function migrateUnprefixTcher(root) {
  let migrated = 0;
  for (const d of PROVIDER_DIRS) {
    const skillsDir = join(root, d, 'skills');
    if (!existsSync(skillsDir)) continue;
    ...
    for (const name of entries) {
      // A prefixed tcher skill is `<prefix>tcher` -- not the canonical
      // `tcher`, and not the legacy `teach-tcher` (cleanup handles that).
      if (name === 'tcher' || name === 'teach-tcher') continue;
      if (!name.endsWith('-tcher')) continue;
      if (!isRealSkillDir(skillsDir, name)) continue;

      const dest = join(skillsDir, 'tcher');
      try {
...
function resolveLinkSource(sourceValue, root) {
  const sourcePath = sourceValue || '.tcher';
...
async function link(flags) {
  ...
  if (!yes) {
    ...
    const ans = await ask(`Link tcher skills into ${targets.length} folder(s)? (Y/n) `);
    if (ans === 'n' || ans === 'no') {
  ...
  console.log(`Linked tcher into: ${targets.join(', ')} (${parts.join(', ')}).`);
...
async function install(flags) {
  ...
  if (!yes) {
    console.log(`Target harness folder(s): ${targets.join(', ')}`);
    const ans = await ask(`Install tcher skills into ${targets.length} folder(s)? (Y/n) `);
    if (ans === 'n' || ans === 'no') {
  ...
  // canonical `tcher` dir instead of orphaning the prefixed one.
  migrateUnprefixTcher(root);
  ...
  console.log(`Installed tcher into: ${targets.join(', ')}`);
  ...
  console.log('\nDone! Run /tcher init in your AI harness to set up design context.\n');
}
...
function findLinkedProviders(root, providers) {
  return providers.filter(provider => {
    const skillDir = join(root, provider, 'skills', 'tcher');
    try {
...
async function update(flags = []) {
  ...
  try {
    ...
    // canonical `tcher` dir rather than orphaning the prefixed copy.
    const migrated = migrateUnprefixTcher(root);
    if (migrated > 0) console.log('Migrated a prefixed install back to /tcher (the i- prefix is no longer used).');
...
export { migrateUnprefixTcher, linkProviderSkills, resolveLinkSource };
...
export async function run(args) {
  ...
  if (!sub || sub === 'help' || sub === '--help' || sub === '-h') {
    ...
    console.error(`Run 'tcher skills --help' for available commands.`);
    process.exit(1);
  }
}
