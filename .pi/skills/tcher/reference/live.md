...
```bash
node .pi/skills/tcher/scripts/live.mjs
```
...
```
LOOP:
  node .pi/skills/tcher/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node .pi/skills/tcher/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .pi/skills/tcher/scripts/live-status.mjs
node .pi/skills/tcher/scripts/live-resume.mjs --id SESSION_ID
node .pi/skills/tcher/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .pi/skills/tcher/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .pi/skills/tcher/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .pi/skills/tcher/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .pi/skills/tcher/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .pi/skills/tcher/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .pi/skills/tcher/scripts/live-server.mjs stop
```
...
```bash
node .pi/skills/tcher/scripts/detect-csp.mjs
```
