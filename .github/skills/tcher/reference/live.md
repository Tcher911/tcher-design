...
```bash
node .github/skills/tcher/scripts/live.mjs
```
...
```
LOOP:
  node .github/skills/tcher/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node .github/skills/tcher/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .github/skills/tcher/scripts/live-status.mjs
node .github/skills/tcher/scripts/live-resume.mjs --id SESSION_ID
node .github/skills/tcher/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .github/skills/tcher/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .github/skills/tcher/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .github/skills/tcher/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .github/skills/tcher/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .github/skills/tcher/scripts/live-server.mjs stop
```
...
```bash
node .github/skills/tcher/scripts/detect-csp.mjs
```
