  ...
  it('covers live chrome, replace, edit, annotations, insert, refresh recovery, accept/discard, and exit', async (t) => {
    ...
    try {
      ...
      const afterExit = await page.evaluate(() => ({
        ...
        hasGlobalBar: Boolean(window.__tcherLiveQuery?.('#tcher-live-global-bar')),
      }));
...
function loadConsolidatedFixture() {
  ...
  fixture.runtime = {
    ...
    stateProbe: {
      ...
      windowProperty: '__tcherStatefulMounts',
    ...
    steer: {
      ...
      expectSelector: 'h1.hero-title[data-tcher-steer="e2e"]',
      expectSourceContains: 'data-tcher-steer="e2e"',
    },
    probe: {
      expectLiveInit: true,
...
async function runReplaceDiscardRecoveryFlow({ page, tmp, evidence }) {
  ...
  assert.match(session.previewFile, /^node_modules\/\.tcher-live\/[^/]+\/manifest\.json$/);
...
async function runEditCopyFlow({ page, tmp, live, evidence }) {
  ...
  assert.doesNotMatch(sourceAfter, /contenteditable|data-tcher-editable|data-tcher-original-text|tcher-variants/, 'manual Apply does not leak live scaffolding');
}
...
async function runAcceptReplaceFlow({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-tcher-variants|tcher-variants-start|data-tcher-variant=/, 'accepted replace source is clean');
...
async function runInsertFlowWithRecovery({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(source, /data-tcher-variants|tcher-variants-start|data-tcher-variant=/, 'accepted insert source is clean');
...
async function assertSvelteAdapterInjection(tmp) {
  ...
  assert.doesNotMatch(appHtml, /live\.js|tcher-live-start/, 'Svelte adapter leaves src/app.html untouched');
  assert.match(layout, /TcherLiveRoot|tcher-live-svelte-start/, 'Svelte adapter patches layout with live root');
}

async function assertShadowChrome(page) {
  const result = await page.evaluate(() => {
    const host = document.getElementById('tcher-live-root');
    const root = host?.shadowRoot;
    return {
      hasHost: Boolean(host),
      hasShadowRoot: Boolean(root),
      hasGlobalBar: Boolean(root?.getElementById('tcher-live-global-bar')),
      documentHasGlobalBar: Boolean(document.querySelector('#tcher-live-global-bar')),
    };
  });
...
async function assertHostileCssIsolation(page) {
  ...
  const hostileStyle = await page.addStyleTag({
    content: `
      button, div, input, svg, #tcher-live-root, tcher-live-root * {
...
async function waitForGlobalBarStable(page) {
  ...
  await page.waitForFunction(
    () => {
      const bar = window.__tcherLiveQuery?.('#tcher-live-global-bar');
      if (!bar) return false;
...
async function assertSelectedElementChrome(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    const bar = q('#tcher-live-bar');
    const picker = q('#tcher-live-picker');
    const badge = q('#tcher-live-edit-badge');
    const highlight = q('#tcher-live-highlight');
    const input = q('#tcher-live-input');
    return {
      hasBar: Boolean(bar),
...
async function selectAction(page, label) {
  ...
  const opened = await page.evaluate(() => {
    const bar = window.__tcherLiveQuery('#tcher-live-bar');
  ...
  await page.waitForFunction(
    (expected) => {
      const picker = window.__tcherLiveQuery('#tcher-live-picker');
...
async function clickEditBadgeAction(page, label) {
  const clicked = await page.evaluate((expected) => {
    const badge = window.__tcherLiveQuery('#tcher-live-edit-badge');
...
async function clickPendingTrash(page) {
  ...
  const clicked = await page.evaluate(() => {
    const dock = window.__tcherLiveQuery('#tcher-live-pending-dock');
...
async function waitForSvelteAcceptComplete(tmp, session, { timeout }) {
  ...
  while (Date.now() < deadline) {
    ...
    if (
      ...
      && !/data-tcher-variants|tcher-variants-start|data-tcher-variant=/.test(source)
    ) {
      return;
    }
...
function assertCompletedRealSource(tmp, id) {
  ...
  assert.match(snapshot.previewFile || '', /node_modules\/\.tcher-live\/[^/]+\/manifest\.json/);
}
...
function readSessionSnapshot(tmp, id) {
  const file = join(tmp, '.tcher/live/sessions', `${id}.snapshot.json`);
...
async function assertNoCyclingZero(page) {
  const text = await page.evaluate(() => window.__tcherLiveQuery('#tcher-live-bar')?.textContent || '');
...
async function readStateProbe(page) {
  return page.evaluate(() => ({
    ...
    mounts: window.__tcherStatefulMounts,
  }));
}
...
function assertFinalSourceClean(tmp) {
  const source = readFileSync(join(tmp, ROUTE_FILE), 'utf-8');
  assert.doesNotMatch(source, /node_modules\/\.tcher-live|data-tcher-variants|tcher-variants-start|data-tcher-variant=/);
}
...
function readJournalEvents(tmp) {
  const dir = join(tmp, '.tcher/live/sessions');
...
function createEvidenceWriter({ page, tmp, live, consoleErrors }) {
  ...
  return {
    async capture(name) {
      ...
      try {
        const dom = await page.evaluate(() => {
          ...
          return {
            ...
            liveState: window.__TCHER_LIVE_CHROME_CORE__?.debugState?.() || null,
            localSession: localStorage.getItem('tcher-live-session'),
          };
        });
      ...
      copyDirIfExists(join(tmp, '.tcher/live/sessions'), join(dir, 'sessions'));
      copyDirIfExists(join(tmp, 'node_modules/.tcher-live'), join(dir, 'tcher-live-preview'));
...
function listPreviewFiles(tmp) {
  const root = join(tmp, 'node_modules/.tcher-live');
...
function createArtifactRoot() {
  const explicit = process.env.TCHER_SVELTE_DEEPSEEK_ARTIFACT_DIR;
