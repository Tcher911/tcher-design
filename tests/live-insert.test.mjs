...
describe('buildInsertWrapperLines', () => {
  it('produces an insert scaffold without an original variant', () => {
    ...
    assert.match(joined, /tcher-variants-start abc12345/);
    assert.match(joined, /data-tcher-variants="abc12345"/);
    assert.match(joined, /data-tcher-mode="insert"/);
    assert.match(joined, /Variants: insert below this line/);
    assert.doesNotMatch(joined, /data-tcher-variant="original"/);
  });
  ...
  it('keeps marker comments inside the wrapper for JSX', () => {
    ...
    const joined = lines.join('\n');
    assert.match(joined, /^\s*<div data-tcher-variants="jsx12345"/m);
    assert.match(joined, /{\/\* tcher-variants-start jsx12345 \*\/}/);
    assert.doesNotMatch(joined, /data-tcher-variant="original"/);
  });
});
...
describe('live-insert CLI integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'tcher-insert-test-'));
  });
...
<body>
  ...
  <section class="features">
    ...
    assert.match(after, /data-tcher-mode="insert"/);
    assert.match(after, /tcher-variants-start ins12345/);
    assert.ok(after.indexOf('class="hero"') < after.indexOf('tcher-variants-start ins12345'));
    assert.ok(after.indexOf('tcher-variants-start ins12345') < after.indexOf('class="features"'));
    assert.doesNotMatch(after, /data-tcher-variant="original"/);
  });
  ...
  <section class="cta">CTA</section>
    ...
    assert.ok(after.indexOf('tcher-variants-start ins99999') < after.indexOf('class="cta"'));
    assert.ok(after.indexOf('class="hero"') < after.indexOf('tcher-variants-start ins99999'));
  });
  ...
  return (
    ...
    assert.match(after, /data-tcher-mode="insert"/);
    assert.match(after, /{\/\* tcher-variants-start jsxins01 \*\/}/);
  });
