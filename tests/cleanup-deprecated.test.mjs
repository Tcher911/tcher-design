...
function makeTmpDir() {
  return mkdtempSync(join(tmpdir(), 'tcher-cleanup-test-'));
}
...
describe('cleanup-deprecated', () => {
  ...
  describe('isTcherSkill', () => {
    it('returns true when SKILL.md mentions tcher', () => {
      const dir = writeSkill(tmp, '.claude', 'arrange', 'Invoke /tcher first.');
      assert.equal(isTcherSkill(dir), true);
    });

    it('returns false when SKILL.md does not mention tcher', () => {
      const dir = writeSkill(tmp, '.claude', 'arrange', 'This is my custom arrange skill.');
      assert.equal(isTcherSkill(dir), false);
    });

    it('returns false for non-existent directory', () => {
      assert.equal(isTcherSkill(join(tmp, 'nope')), false);
    });

    it('returns true when lock source says Tcher911/tcher-design, even if SKILL.md never mentions it', () => {
      const dir = writeSkill(tmp, '.claude', 'harden', 'A custom skill with no pack mention.');
      const lock = {
        skills: { harden: { source: 'Tcher911/tcher-design' } },
      };
      assert.equal(isTcherSkill(dir, { skillName: 'harden', lock }), true);
    });
    ...
    it('returns false when lock source is a different pack', () => {
      ...
      assert.equal(isTcherSkill(dir, { skillName: 'harden', lock }), false);
    });

    it('falls back to SKILL.md content when no lock entry exists', () => {
      const dir = writeSkill(tmp, '.claude', 'harden', 'Invoke /tcher to harden.');
      const lock = { skills: {} };
      assert.equal(isTcherSkill(dir, { skillName: 'harden', lock }), true);
    });
  });
  ...
  describe('loadLock', () => {
    ...
    it('parses skills-lock.json when present', () => {
      const lock = { version: 1, skills: { arrange: { source: 'Tcher911/tcher-design' } } };
  ...
  describe('removeDeprecatedSkills', () => {
    it('deletes tcher-owned deprecated skill directories', () => {
      writeSkill(tmp, '.claude', 'arrange', 'Invoke /tcher first.');
      writeSkill(tmp, '.claude', 'normalize', 'Run tcher teach.');
    ...
    it('does NOT delete skills that do not mention tcher', () => {
    ...
    it('deletes i-prefixed variants', () => {
      writeSkill(tmp, '.cursor', 'i-normalize', 'Invoke /tcher first.');
      const deleted = removeDeprecatedSkills(tmp);
    ...
    it('cleans across multiple harness directories', () => {
      writeSkill(tmp, '.claude', 'onboard', 'Run tcher teach first.');
      writeSkill(tmp, '.agents', 'onboard', 'Run tcher teach first.');
      writeSkill(tmp, '.cursor', 'onboard', 'Run tcher teach first.');
      const deleted = removeDeprecatedSkills(tmp);
      assert.equal(deleted.length, 3);
    });

    it('leaves non-deprecated skills alone', () => {
      writeSkill(tmp, '.claude', 'my-custom-skill', 'Invoke /tcher first.');
      writeSkill(tmp, '.claude', 'arrange', 'Invoke /tcher first.');
      const deleted = removeDeprecatedSkills(tmp);
    ...
    it('deletes a stock v2.x harden skill with no lock file via the fingerprint fallback', () => {
      ...
      // v2.x harden SKILL.md never contained the word "tcher", but
    ...
    it('deletes skills whose SKILL.md never mentions tcher when the lock claims them', () => {
      // Reproduces the "orphan dir" bug: the old SKILL.md bodies described
      // each skill on its own merits and never said the word "tcher",
      ...
      const lock = {
        version: 1,
        skills: { harden: { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'x' } },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
      const deleted = removeDeprecatedSkills(tmp);
      assert.equal(deleted.length, 1);
    ...
    it('handles symlinks to deprecated skills', () => {
      ...
      const canonical = writeSkill(tmp, '.agents', 'extract', 'Use tcher extract.');
  ...
  describe('cleanSkillsLock', () => {
    it('removes tcher-owned deprecated entries', () => {
      const lock = {
        version: 1,
        skills: {
          arrange: { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'abc' },
          tcher: { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'def' },
          'resolve-reviews': { source: 'acme/agent-reviews', sourceType: 'github', computedHash: 'ghi' },
        },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
      ...
      assert.ok(updated.skills.tcher); // not deprecated
    ...
    it('removes i-prefixed entries', () => {
      const lock = {
        version: 1,
        skills: {
          'i-arrange': { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'abc' },
          'i-normalize': { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'def' },
        },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
      const removed = cleanSkillsLock(tmp);
  ...
  describe('cleanup (integration)', () => {
    it('cleans both files and lock entries in one pass', () => {
      ...
      writeSkill(tmp, '.claude', 'arrange', 'Invoke /tcher.');
      writeSkill(tmp, '.agents', 'arrange', 'Invoke /tcher.');
      writeSkill(tmp, '.claude', 'extract', 'Run tcher extract.');

      // Set up lock file
      const lock = {
        version: 1,
        skills: {
          arrange: { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'a' },
          extract: { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'b' },
          tcher: { source: 'Tcher911/tcher-design', sourceType: 'github', computedHash: 'c' },
        },
      };
      writeFileSync(join(tmp, 'skills-lock.json'), JSON.stringify(lock), 'utf-8');
    ...
    it('is a no-op when nothing needs cleaning', () => {
      writeSkill(tmp, '.claude', 'my-custom-skill', 'Invoke /tcher.');
      const result = cleanup(tmp);
