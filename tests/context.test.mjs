 ...
 *   3. TCHER_CONTEXT_DIR env var as a power-user escape hatch (only
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'tcher-loadctx-'));
  savedEnv = process.env.TCHER_CONTEXT_DIR;
  delete process.env.TCHER_CONTEXT_DIR;
});

afterEach(() => {
  if (savedEnv === undefined) delete process.env.TCHER_CONTEXT_DIR;
  else process.env.TCHER_CONTEXT_DIR = savedEnv;
...
describe('resolveContextDir', () => {
  ...
  it('uses TCHER_CONTEXT_DIR as a fallback when defaults are empty (relative path)', () => {
    write('design/PRODUCT.md');
    process.env.TCHER_CONTEXT_DIR = 'design';
    assert.equal(resolveContextDir(scratch), path.join(scratch, 'design'));
  });

  it('uses TCHER_CONTEXT_DIR as a fallback when defaults are empty (absolute path)', () => {
    const elsewhere = fs.mkdtempSync(path.join(os.tmpdir(), 'tcher-elsewhere-'));
    try {
      process.env.TCHER_CONTEXT_DIR = elsewhere;
  ...
  it('default paths win over TCHER_CONTEXT_DIR (lazy escape hatch)', () => {
  ...
  it('ignores empty TCHER_CONTEXT_DIR', () => {
    write('PRODUCT.md');
    process.env.TCHER_CONTEXT_DIR = '   ';
    assert.equal(resolveContextDir(scratch), scratch);
  });
...
describe('loadContext (TCHER_CONTEXT_DIR escape hatch)', () => {
  ...
  it('reports a missing override directory as no-context, not as a crash', () => {
    process.env.TCHER_CONTEXT_DIR = 'no/such/dir';
    const ctx = loadContext(scratch);
...
describe('context.mjs CLI', () => {
  it('emits NO_PRODUCT_MD directive when no PRODUCT.md is found', async () => {
    ...
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, TCHER_NO_UPDATE_CHECK: '1' } });
...
describe('context.mjs update check', () => {
  ...
  function setup(cacheObj, { disable = false, host } = {}) {
    ...
    const env = {
      ...process.env,
      TCHER_UPDATE_CACHE: cachePath(),
      TCHER_NO_UPDATE_CHECK: disable ? '1' : '',
      ...(host ? { TCHER_UPDATE_HOST: host } : {}),
    };
  ...
  it('respects TCHER_NO_UPDATE_CHECK', () => {
