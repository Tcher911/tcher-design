...
describe('live reference authoring contract', () => {
  it('keeps the live prompt focused on the foreground poll loop', () => {
    ...
    const manualAgentMd = readFileSync(join(ROOT, 'skill/agents/tcher-manual-edit-applier.md'), 'utf-8');
    ...
    assert.doesNotMatch(liveMd, /TCHER_LIVE_COPY_AGENT|mock/);
    ...
    assert.match(liveMd, /delegate source edits to `tcher_manual_edit_applier`/);
    ...
    assert.match(manualAgentMd, /codex-name: tcher_manual_edit_applier/);
