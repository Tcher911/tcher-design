...
export async function runPreActions(page, actions) {
  ...
  const wasActive = await page.evaluate((sel) =>
    window.__tcherLiveQuery?.(sel)?.dataset.active === 'true',
...
async function liveUiSnapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      hasShadowRoot: Boolean(document.getElementById('tcher-live-root')?.shadowRoot),
      ...
      wrapper: wrapper ? { preview: wrapper.dataset.tcherPreview, count: wrapper.dataset.tcherVariantCount, html: wrapper.outerHTML.slice(0, 800) } : null,
