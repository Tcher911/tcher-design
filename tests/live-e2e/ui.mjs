 ...
 *
 * Selector strategy: live-browser.js uses deterministic ids (`tcher-live-*`)
...
const BAR_ID = '#tcher-live-bar';
const GLOBAL_BAR_ID = '#tcher-live-global-bar';
const PICKER_ID = '#tcher-live-picker';
const EDIT_BADGE_ID = '#tcher-live-edit-badge';
const PENDING_DOCK_ID = '#tcher-live-pending-dock';
const STEER_CHAT_ID = '#tcher-live-page-chat';
const STEER_INPUT_ID = '#tcher-live-page-chat-input';
const PICK_TOGGLE = '#tcher-live-pick-toggle';
...
const INSERT_TOGGLE = '#tcher-live-insert-toggle';
const DETECT_TOGGLE = '#tcher-live-detect-toggle';
const DETECT_BADGE = '#tcher-live-detect-badge';
const DESIGN_TOGGLE = '#tcher-live-design-toggle';
const DESIGN_HOST = '#tcher-live-design-host';
const EXIT_BUTTON = '#tcher-live-exit';
const INSERT_INPUT_ID = '#tcher-live-insert-input';
const INSERT_CREATE_ID = '#tcher-live-insert-create';
const ANNOTATION_ID = '#tcher-live-annot';
const ANNOTATION_PINS_ID = '#tcher-live-annot-pins';
const ANNOTATION_CLEAR_ID = '#tcher-live-annot-clear';

/**
 * Wait for the live handshake to complete:
 *   - window.__TCHER_LIVE_INIT__ set
 *   - global bar mounted
...
export async function waitForHandshake(page, { timeout = 20_000 } = {}) {
  ...
  await page.waitForFunction(
    (sel) => Boolean(window.__tcherLiveQuery?.(sel)),
    GLOBAL_BAR_ID,
    { timeout },
  );
...
export async function assertBottomBarIdle(page, { timeout = 5_000 } = {}) {
  await installLiveQueryHelpers(page);
  await page.waitForFunction(
    ({ ids }) => ids.every((sel) => Boolean(window.__tcherLiveQuery(sel))),
    {
      ids: [
        GLOBAL_BAR_ID,
        PICK_TOGGLE,
  ...
  const snapshot = await page.evaluate(({ pickSel, insertSel, detectSel, designSel }) => {
    const q = window.__tcherLiveQuery;
    return {
...
function installLiveQueryHelpersInPage() {
  window.__tcherLiveQuery = (selector) => {
    const root = window.__TCHER_LIVE_CHROME_CORE__?.root?.()
      || window.__TCHER_LIVE_UI_ROOT__
      || null;
    return root?.querySelector?.(selector) || document.querySelector(selector);
  };
  window.__tcherLiveQueryAll = (selector) => {
    const root = window.__TCHER_LIVE_CHROME_CORE__?.root?.()
      || window.__TCHER_LIVE_UI_ROOT__
      || null;
...
async function clickLiveControl(page, selector) {
  ...
  const clicked = await page.evaluate((sel) => {
    const el = window.__tcherLiveQuery(sel);
...
async function readControlActive(page, selector) {
  await installLiveQueryHelpers(page);
  return page.evaluate((sel) => window.__tcherLiveQuery(sel)?.dataset.active === 'true', selector);
}
...
async function ensureLiveControlActive(page, selector, active) {
  ...
  await page.waitForFunction(
    ({ sel, expected }) => window.__tcherLiveQuery(sel)?.dataset.active === (expected ? 'true' : 'false'),
...
export async function runPickInsertToggleSmoke(page) {
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__tcherLiveQuery(pickSel)?.dataset.active === 'true'
      && window.__tcherLiveQuery(insertSel)?.dataset.active === 'false',
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__tcherLiveQuery(pickSel)?.dataset.active === 'false'
      && window.__tcherLiveQuery(insertSel)?.dataset.active === 'true',
    { pickSel: PICK_TOGGLE, insertSel: INSERT_TOGGLE },
    { timeout: 5_000 },
  );
...
export async function runDetectSmoke(page, { expectMinCount = 1 } = {}) {
  ...
  await page.waitForFunction(
    ({ badgeSel, expectMin }) => {
      const badge = window.__tcherLiveQuery(badgeSel);
      const count = parseInt(badge?.textContent || '0', 10);
      const overlays = document.querySelectorAll('.tcher-overlay').length;
  ...
  await ensureLiveControlActive(page, PICK_TOGGLE, true);
  await page.waitForFunction(
    () => [...document.querySelectorAll('.tcher-overlay')]
  ...
  await page.waitForFunction(
    ({ badgeSel }) => {
      const badge = window.__tcherLiveQuery(badgeSel);
      return document.querySelectorAll('.tcher-overlay').length === 0
...
export async function runDesignPanelSmoke(page, { title = '', rawText = '' } = {}) {
  ...
  await page.waitForFunction(
    ({ hostSel, titleText }) => {
      const host = window.__tcherLiveQuery(hostSel);
  ...
  await page.evaluate((hostSel) => {
    const root = window.__tcherLiveQuery(hostSel)?.shadowRoot;
    const raw = [...(root?.querySelectorAll('.tab') || [])].find((btn) => /Raw/i.test(btn.textContent || ''));
    raw?.click();
  }, DESIGN_HOST);
  await page.waitForFunction(
    ({ hostSel, expected }) => {
      const text = window.__tcherLiveQuery(hostSel)?.shadowRoot?.textContent || '';
  ...
  await page.waitForFunction(
    ({ hostSel, toggleSel }) => {
      const panel = window.__tcherLiveQuery(hostSel)?.shadowRoot?.querySelector('.panel');
      const toggle = window.__tcherLiveQuery(toggleSel);
...
export async function clickExitLiveMode(page) {
  ...
  await page.waitForFunction(
    ({ barSel }) => {
      const bar = window.__tcherLiveQuery?.(barSel);
      return window.__TCHER_LIVE_INIT__ === false && (!bar || !bar.isConnected);
    },
...
export async function drawAnnotationPinAndStroke(page, {
  ...
  await page.waitForFunction(
    (pinsSel) => Boolean(window.__tcherLiveQuery(pinsSel)?.querySelector('input')),
    ANNOTATION_PINS_ID,
    { timeout: 5_000 },
  );
  await page.evaluate(({ pinsSel, value }) => {
    const input = window.__tcherLiveQuery(pinsSel)?.querySelector('input');
  ...
  await page.waitForFunction(
    ({ pinsSel, expected }) => {
      const pins = window.__tcherLiveQuery(pinsSel);
  ...
  await page.waitForFunction(
    ({ annotSel, clearSel }) => {
      const annot = window.__tcherLiveQuery(annotSel);
      const clear = window.__tcherLiveQuery(clearSel);
...
async function waitForAnnotationRect(page) {
  ...
  return page.evaluate((sel) => {
    const rect = window.__tcherLiveQuery(sel).getBoundingClientRect();
    return {
      left: rect.left,
      top: rect.top,
      width: rect.width,
...
async function hideAnnotationOverlay(page) {
  await page.evaluate(() => {
    const annot = window.__tcherLiveQuery('#tcher-live-annot');
...
async function ensurePickerActive(page) {
  ...
  const clicked = await page.evaluate((sel) => {
    const btn = window.__tcherLiveQuery(sel);
    if (!btn) return false;
    btn.click();
    return true;
  }, PICK_TOGGLE_ID);
  ...
  await page.waitForFunction(
    (sel) => window.__tcherLiveQuery(sel)?.dataset.active === 'true',
    PICK_TOGGLE_ID,
    { timeout: 5_000 },
  );
}
...
export async function waitForCycling(page, expectedCount, { timeout = 30_000 } = {}) {
  ...
  try {
    ...
    if (process.env.TCHER_E2E_DEBUG) {
      const snapshot = await page.evaluate((barSel) => {
        const query = window.__tcherLiveQuery || ((sel) => document.querySelector(sel));
        const root = window.__TCHER_LIVE_CHROME_CORE__?.root?.() || window.__TCHER_LIVE_UI_ROOT__ || null;
        const bar = query(barSel);
        const toast = query('#tcher-live-toast');
        const wrapper = document.querySelector('[data-tcher-variants]');
        return {
          liveInit: window.__TCHER_LIVE_INIT__,
          adapter: window.__TCHER_LIVE_ADAPTER__,
          ...
          wrapper: wrapper ? { preview: wrapper.dataset.tcherPreview, count: wrapper.dataset.tcherVariantCount, html: wrapper.outerHTML.slice(0, 600) } : null,
          debugState: window.__TCHER_LIVE_CHROME_CORE__?.debugState?.() || null,
          storage: localStorage.getItem('tcher-live-session'),
          scripts: document.querySelectorAll('script[data-tcher-live-script]').length,
...
export async function getVisibleVariant(page) {
  ...
  return page.evaluate((barSel) => {
    const wrapper = window.__tcherLiveQuery('[data-tcher-variants]');
    if (wrapper) {
      ...
      const idx = visible ? parseInt(visible.dataset.tcherVariant || '0', 10) : 0;
      if (idx > 0) return idx;
    }
    const bar = window.__tcherLiveQuery(barSel);
    if (!bar) return null;
...
export async function clickEditCopy(page) {
  ...
  await page.waitForFunction(
    () => window.__tcherLiveQuery('[data-tcher-editable="true"]')?.isContentEditable === true,
    { timeout: 5_000 },
  );
}
...
export async function clickSaveEdit(page) {
  ...
  await page.waitForFunction(
    () => !window.__tcherLiveQuery('[data-tcher-editable="true"]'),
    { timeout: 5_000 },
  );
}
...
async function clickEditBadgeButton(page, label) {
  const proxyRect = await page.evaluate((text) => {
    const proxies = [...document.querySelectorAll('[data-tcher-edit-badge-proxy="true"]')];
...
export async function assertApplyDockVisible(page, expectedCount, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ dockSel, expected }) => {
      const dock = window.__tcherLiveQuery(dockSel);
...
export async function preparePageForBarInteraction(page) {
  await page.evaluate(() => {
    for (const el of window.__tcherLiveQueryAll('astro-dev-toolbar')) {
...
export async function waitForSteerInputFocused(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (inputSel) => {
      const input = window.__tcherLiveQuery(inputSel);
      const active = window.__TCHER_LIVE_CHROME_CORE__?.activeElementDeep?.()
...
export async function waitForSteerInputValue(page, value, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ inputSel, value: expected }) => window.__tcherLiveQuery(inputSel)?.value === expected,
...
export async function waitForSteerLocked(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (sel) => window.__tcherLiveQuery(sel)?.dataset.processing === 'true',
    STEER_CHAT_ID,
    { timeout },
  );
}
...
export async function waitForSteerUnlocked(page, { timeout = 15_000 } = {}) {
  await page.waitForFunction(
    (sel) => {
      const chat = window.__tcherLiveQuery(sel);
      const input = window.__tcherLiveQuery('#tcher-live-page-chat-input');
...
async function ensureToggleActive(page, selector, shouldBeActive) {
  ...
  await page.locator(selector).click({ timeout: 5_000 });
  await page.waitForFunction(
    ({ sel, active }) => window.__tcherLiveQuery(sel)?.dataset.active === (active ? 'true' : 'false'),
...
export async function runInsertFlow(page, {
  ...
  await page.waitForFunction(() => {
    const line = window.__tcherLiveQuery('#tcher-live-insert-line');
  ...
  await page.waitForFunction(
    ({ sel, value }) => window.__tcherLiveQuery(sel)?.value === value,
