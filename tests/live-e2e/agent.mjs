 ...
 *    `<style data-tcher-css="ID">` block with `@scope ([data-tcher-variant="N"])`
 *    rules, a `data-tcher-params` JSON manifest covering range + steps + toggle
...
export const STEER_MARKER_ATTR = 'data-tcher-steer';
 ...
 * @property {string}        scopedCss   Contents of the <style data-tcher-css>
 ...
 *   - <style data-tcher-css="ID"> with @scope rules per variant
 *   - data-tcher-params manifest with range + steps + toggle kinds
  ...
  return {
    ...
    async generateVariants(event, context = {}) {
      ...
      const scopedCss = useAstroGlobalCss
        ? [
            `[data-tcher-variant="1"] > ${tag} {`,
            '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
            '}',
            `[data-tcher-variant="2"] > ${tag} { font-weight: 900; }`,
            `[data-tcher-variant="2"][data-p-face="serif"] > ${tag} { font-family: ui-serif, serif; }`,
            `[data-tcher-variant="2"][data-p-face="mono"]  > ${tag} { font-family: ui-monospace, monospace; }`,
            `[data-tcher-variant="3"] > ${tag} { text-transform: uppercase; letter-spacing: 0.04em; }`,
            `[data-tcher-variant="3"][data-p-italic] > ${tag} { font-style: italic; }`,
          ].join('\n')
        : [
...
function generateInsertFakeVariants(context = {}) {
  ...
  const scopedCss = useAstroGlobalCss
    ? [
        '[data-tcher-variant="1"] .inserted-copy {',
        '  color: oklch(var(--p-lightness, 0.5) 0.25 25);',
        '}',
        '[data-tcher-variant="2"] .inserted-copy { font-weight: 900; }',
        '[data-tcher-variant="2"][data-p-face="serif"] .inserted-copy { font-family: ui-serif, serif; }',
        '[data-tcher-variant="2"][data-p-face="mono"]  .inserted-copy { font-family: ui-monospace, monospace; }',
        '[data-tcher-variant="3"] .inserted-copy { text-transform: uppercase; letter-spacing: 0.04em; }',
        '[data-tcher-variant="3"][data-p-italic] .inserted-copy { font-style: italic; }',
      ].join('\n')
    : [
        '@scope ([data-tcher-variant="1"]) {',
        '  :scope .inserted-copy {',
...
export const HOIST_ATTR = 'data-tcher-hoist-id';
...
function normalizeVariantSelectorQuotes(css) {
  return String(css).replace(
    /\[data-tcher-variant=(['"])(\d+)\1\]/g,
    (_match, _quote, id) => `[data-tcher-variant="${id}"]`,
  );
}
...
function hasBaseVariantRule(scopedCss, variantId, styleMode) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return new RegExp(String.raw`\[data-tcher-variant=${q}\](?:\s|>|\.|#|\[${HOIST_ATTR}=)`).test(scopedCss);
  }
  return new RegExp(String.raw`@scope\s*\(\s*\[data-tcher-variant=${q}\]\s*\)`).test(scopedCss);
}

function renderBaseVariantRule(variantId, styleMode) {
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-tcher-variant="${variantId}"] > * {`,
      '  --tcher-variant-ready: 1;',
      '}',
    ].join('\n');
  }
  return [
    `@scope ([data-tcher-variant="${variantId}"]) {`,
    '  :scope > * { --tcher-variant-ready: 1; }',
    '}',
  ].join('\n');
}
...
// `style="..."`, and tag the element with `data-tcher-hoist-id="N"`.
...
function renderHoistedInlineStyleRule({ variantId, hoistId, declarations, styleMode }) {
  ...
  if (styleMode === 'astro-global-prefixed') {
    return [
      `[data-tcher-variant="${variantId}"] ${target} {`,
 ...
 *   - data-tcher-params stays a single-quoted JSON string (JSX-legal)
 */
function renderVariantsBlock({ sessionId, indent, output, commentSyntax, file, styleMode }) {
  ...
  const styleLines = isJsx
    ? [
        indent + '  <style data-tcher-css="' + sessionId + '">{`',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  `}</style>',
      ]
    : [
        indent + '  <style' + (isAstroGlobalCss ? ' is:inline' : '') + ' data-tcher-css="' + sessionId + '">',
        ...output.scopedCss.split('\n').map((l) => indent + '    ' + l),
        indent + '  </style>',
      ];
  ...
  const variantBlocks = output.variants.map((v, i) => {
    ...
    const paramsAttr = v.params && v.params.length
      ? " data-tcher-params='" + attrEscape(JSON.stringify(v.params), { svelte: isSvelte }) + "'"
      : '';
    let styleAttr = '';
    ...
    return [
      ...
      indent + '  <div data-tcher-variant="' + idx + '"' + styleAttr + paramsAttr + '>',
      indent + '    ' + inner,
      indent + '  </div>',
    ].join('\n');
  });
...
async function writeSvelteComponentVariants({ tmp, wrapInfo, event, output }) {
  ...
  for (let i = 0; i < output.variants.length; i++) {
    ...
    if (isInsert && /\bdata-tcher-[\w-]*\s*=/.test(markup)) {
      throw new Error(`Svelte insert variant ${variantId} contains preview-only data-tcher attributes`);
    }
...
function svelteCssForVariant(scopedCss, variantId, tag) {
  ...
  const rewritten = chunks
    .join('\n')
    .replace(new RegExp(String.raw`\\[data-tcher-variant=["']${variantId}["']\\]\\s*>\\s*`, 'g'), '')
    .replace(new RegExp(String.raw`\\[data-tcher-variant=["']${variantId}["']\\][^{]*>\\s*`, 'g'), '')
    .replace(/:scope(?:\[[^\]]+\])?\s*>\s*/g, '')
    .replace(/:scope(?:\[[^\]]+\])?/g, tag)
    .split('\n')
...
function extractVariantCssChunks(css, variantId) {
  ...
  for (const line of lines) {
    if (line.includes(`[data-tcher-variant="${variantId}"]`) || line.includes(`[data-tcher-variant='${variantId}']`)) {
      collecting = true;
      depth = 0;
 ...
 * @param {string} opts.scriptsDir Path to the tcher scripts dir.
 ...
 *
 *   1. Locate the carbonize block (bracketed by `tcher-carbonize-start`
 *      and `tcher-carbonize-end`).
 ...
 *      `<div data-tcher-variant="N" style="display: contents"|...>` wrapper
...
async function runCarbonizeCleanup({ tmp, file, sessionId /* , variant */ }) {
  ...
  const startRe = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*tcher-carbonize-start\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n');
  const endRe   = new RegExp('[ \\t]*(?:<!--|\\{/\\*)\\s*tcher-carbonize-end\\s+' + sessionId + '\\s*(?:-->|\\*/\\})\\n?');
  ...
  // 2. Unwrap the temporary `<div data-tcher-variant="N" ...>` placed
  ...
  // followed by `</div>`, where the open carries data-tcher-variant
  // and is NOT inside a data-tcher-variants wrapper (the variants
  // wrapper has the trailing `s`).
  body = body.replace(
    /^([ \t]*)<div\b[^>]*\bdata-tcher-variant="[^"]+"[^>]*>\n([\s\S]*?)\n[ \t]*<\/div>\n/m,
    (match, indent, inner) => {
  ...
  // 3. Strip any `data-tcher-hoist-id` attributes the normalize step
  ...
  body = body.replace(/\s+data-tcher-hoist-id="[^"]*"/g, '');
