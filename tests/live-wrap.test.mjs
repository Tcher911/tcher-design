...
describe('findElement', () => {
  ...
  it('skips lines that contain data-tcher-variant', () => {
    const lines = [
      '<div class="hero" data-tcher-variant="original">Old</div>',
...
describe('wrapCli integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'tcher-wrap-test-'));
...
<body>
  <div class="hero-section">
    ...
    assert.ok(modified.includes('data-tcher-variants="test123"'));
    assert.ok(modified.includes('data-tcher-variant-count="3"'));
    assert.ok(modified.includes('data-tcher-variant="original"'));
    assert.ok(modified.includes('display: contents'));
    assert.ok(modified.includes('tcher-variants-start test123'));
    assert.ok(modified.includes('tcher-variants-end test123'));
    // Original should NOT be hidden (stays visible until variants arrive)
    assert.ok(!modified.includes('data-tcher-variant="original" style="display: none"'));
  });
  ...
  return (
    ...
    assert.ok(modified.includes('{/* tcher-variants-start jsx123'));
    assert.ok(modified.includes('data-tcher-variant-count="2"'));
  });
...
<div id="pricing">
  ...
  <p>Plans start at $10/mo.</p>
    ...
    assert.ok(modified.includes('data-tcher-variants="id123"'));
...
<div class="after">After</div>`;
    ...
    assert.ok(modified.includes('data-tcher-variants="pres123"'));
  });
  ...
  <h1>Plain title</h1>
    ...
    assert.equal(result.cssAuthoring.styleTag, '<style data-tcher-css="SESSION_ID">');
...
<section class="hero-shell">
  <h1>{title}</h1>
    ...
    assert.deepEqual(result.cssSelectorPrefixExamples, [
      '[data-tcher-variant="1"]',
      '[data-tcher-variant="2"]',
      '[data-tcher-variant="3"]',
    ]);
    assert.equal(result.cssAuthoring.mode, 'astro-global-prefixed');
    assert.equal(result.cssAuthoring.strategy, 'global-prefixed');
    assert.equal(result.cssAuthoring.styleTag, '<style is:inline data-tcher-css="SESSION_ID">');
    assert.match(result.cssAuthoring.rulePattern, /^\[data-tcher-variant="N"\]/);
    assert.ok(
...
// .tcher/live/pending-manual-edits.json from local dev tripped the
// fail-loud check in live-wrap. Clear the buffer around each test.
function clearManualEditsBuffer() {
  try {
    const p = join(process.cwd(), '.tcher/live/pending-manual-edits.json');
    rmSync(p, { force: true });
  } catch {}
}
...
describe('live-wrap — JSX / TSX correctness', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tcher-wrap-jsx-')); clearManualEditsBuffer(); });
  ...
  return (
    ...
    assert.ok(modified.includes('tcher-variants-start wrapA'), 'wrapper was created');
    ...
    const originalMatch = modified.match(/data-tcher-variant="original"[^>]*>([\s\S]*?)\s*<\/div>/);
...
export default function App() {
  ...
  return (
    ...
    // The outer wrapper <div data-tcher-variants="..."> appears BEFORE
    // both marker comments — markers are tucked inside.
    const wrapperIdx = modified.indexOf('data-tcher-variants="frag1"');
    const startMarkerIdx = modified.indexOf('tcher-variants-start frag1');
    const endMarkerIdx = modified.indexOf('tcher-variants-end frag1');
  ...
  it('HTML wrapper keeps marker comments OUTSIDE the wrapper <div> (existing layout)', () => {
    ...
    const modified = readFileSync(join(tmp, 'page.html'), 'utf-8');
    const wrapperIdx = modified.indexOf('data-tcher-variants="htmlFrag"');
    const startMarkerIdx = modified.indexOf('tcher-variants-start htmlFrag');
  ...
  return (
    ...
    const modified = readFileSync(join(tmp, 'Page.tsx'), 'utf-8');
    const originalMatch = modified.match(/data-tcher-variant="original"[\s\S]*?<\/div>/);
  ...
  return (
    ...
    assert.ok(modified.includes('data-tcher-variants="short1"'),
  ...
  <section class="multiline-target">
    ...
    // line (the tcher-variants-end marker for HTML).
    assert.match(lines[result.endLine - 1], /tcher-variants-end ml1/,
  ...
  return (
    ...
    assert.ok(modified.includes('data-tcher-variants="dyn1"'), 'wrapped (first-match fallback)');
  });
