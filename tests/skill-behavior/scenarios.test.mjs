...
const CRAFT_PROMPT = '/tcher craft a landing page for the project in this workspace';
const PRIMER_PROMPT =
  'Take a quick look at the project. What register is this? Run the tcher context loader once if you need to.';

const VERBOSE = process.env.TCHER_SKILL_BEHAVIOR_VERBOSE === '1';
...
for (const modelId of resolveModelList()) {
  ...
  describe(`skill behavior :: ${modelId}`, () => {
    ...
    it('scenario 4: context already loaded in prior turn', async () => {
      ...
      try {
        ...
        const turn2 = await runTurn({
          workspace,
          model,
          userPrompt: 'Now, /tcher craft a landing page based on what you saw.',
    ...
    it('scenario 7: sub-command routing (`/tcher audit` loads audit.md)', async () => {
      const workspace = prepareWorkspace({
        files: {
          'PRODUCT.md': PRODUCT_MD_SAMPLE,
          'DESIGN.md': DESIGN_MD_SAMPLE,
    ...
    it('scenario 9: update-available directive is surfaced, never auto-run', async () => {
      ...
      const workspace = prepareWorkspace({
        files: {
          'PRODUCT.md': PRODUCT_MD_SAMPLE,
          'index.html': MINIMAL_LANDING_HTML,
          '.tcher-update.json': JSON.stringify({ lastCheck: Date.now(), latestVersion: '99.0.0' }),
        },
        skillVersion: '3.5.0',
      });
      try {
