 ...
 *   2. Symlinks the real .claude/skills/tcher into the workspace so
...
function loadSkillBody() {
  ...
  // and the scripts the skill references live at .claude/skills/tcher/
  ...
  md = md
    ...
    .replaceAll('{{scripts_path}}', '.claude/skills/tcher/scripts')
 ...
 *
 * - `.claude/skills/tcher` is symlinked at the SOURCE skill dir (not
 *   the built `.claude/skills/tcher/`) so the test exercises whatever
...
export function prepareWorkspace({ files = {}, skillVersion = null } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tcher-skill-test-'));
  const skillDest = path.join(dir, '.claude', 'skills', 'tcher');
...
export function makeTools(workspace, extraEnv = {}) {
  ...
  const tools = {
    bash: tool({
      description:
        'Run a bash command in the workspace root. Use this to invoke skill scripts (e.g. `node .claude/skills/tcher/scripts/load-context.mjs`).',
