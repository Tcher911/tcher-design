 ...
 * exposes the accepted element under stale data-tcher runtime wrappers.
 *
 * Run with:
...
describe('live-e2e accept cleanup regression', () => {
  it('cleans accepted LLM DOM wrappers after carbonize cleanup', async (t) => {
    ...
    const llmConfig = resolveLlmAgentConfig({
      provider: process.env.TCHER_E2E_LLM_PROVIDER || 'deepseek',
    });
    ...
    try {
      ...
      assert.doesNotMatch(finalSource, /data-tcher-variants="/, 'source variants wrapper removed');
      assert.doesNotMatch(finalSource, /tcher-variants-start/, 'source variants markers removed');
      assert.doesNotMatch(finalSource, /tcher-carbonize-start/, 'source carbonize markers removed');
      assert.doesNotMatch(finalSource, /data-tcher-variant="/, 'source variant wrapper removed');
    ...
    await page.waitForFunction(
      (sel) => {
        ...
        return matches.length > 0
          && matches.every((el) => !el.closest('[data-tcher-variants],[data-tcher-variant]'));
      },
      selector,
      { timeout: timeoutMs },
    );
...
async function collectAcceptedDomDiagnostic(page, selector) {
  return page.evaluate((sel) => {
    const matches = [...document.querySelectorAll(sel)];
    const clean = matches.filter((el) => !el.closest('[data-tcher-variants],[data-tcher-variant]'));
    const stale = matches.filter((el) => el.closest('[data-tcher-variants],[data-tcher-variant]'));
    const local = {};
    for (const key of [
      'tcher-live-session',
      'tcher-live-session-handled',
      'tcher-live-session-scroll',
    ]) {
    ...
    return {
      ...
      remainingVariantsWrapperCount: document.querySelectorAll('[data-tcher-variants]').length,
      remainingVariantWrapperCount: document.querySelectorAll('[data-tcher-variant]').length,
      liveBarText: document.querySelector('#tcher-live-bar')?.textContent || '',
      localStorage: local,
      heroHtml: matches.map((el) => el.outerHTML.slice(0, 500)),
      staleAncestorHtml: stale.map((el) => {
        const ancestor = el.closest('[data-tcher-variants],[data-tcher-variant]');
...
function isSourceClean(source) {
  return !source.includes('data-tcher-variants=')
    && !source.includes('tcher-variants-start')
    && !source.includes('tcher-carbonize-start')
    && !source.includes('data-tcher-variant=');
}
