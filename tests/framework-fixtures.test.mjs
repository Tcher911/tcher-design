...
function stageFixture(name) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'tcher-fixture-'));
  cpSync(join(fixtureRoot, 'files'), tmp, { recursive: true });
  writeFileSync(join(tmp, '.gitignore'), gitignore);
  mkdirSync(join(tmp, '.tcher', 'live'), { recursive: true });
  writeFileSync(join(tmp, '.tcher', 'live', 'config.json'), JSON.stringify(fixture.config));
    ...
    it('live-inject --port adds the script tag to every config file', () => {
      ...
      try {
        ...
        const ignored = execFileSync('git', [
          ...
          '.tcher/live/sessions/example.jsonl',
          '.tcher/live/previews/example/v1.html',
          '.tcher/live/deferred-svelte-component-accepts.json',
          'src/lib/tcher/TcherLiveRoot.svelte',
          'src/lib/tcher/__runtime.js',
          'src/lib/tcher/a4ac4e74/v3.svelte',
        ], { cwd: tmp, encoding: 'utf-8' });
        assert.match(ignored, /\.tcher\/live\/server\.json/);
        assert.match(ignored, /\.tcher\/live\/sessions\/example\.jsonl/);
        assert.match(ignored, /\.tcher\/live\/previews\/example\/v1\.html/);
        assert.match(ignored, /\.tcher\/live\/deferred-svelte-component-accepts\.json/);
        assert.match(ignored, /src\/lib\/tcher\/TcherLiveRoot\.svelte/);
        assert.match(ignored, /src\/lib\/tcher\/__runtime\.js/);
        assert.match(ignored, /src\/lib\/tcher\/a4ac4e74\/v3\.svelte/);
        if (result.adapter === 'sveltekit') {
          ...
          const root = readFileSync(join(tmp, 'src/lib/tcher/TcherLiveRoot.svelte'), 'utf-8');
          assert.match(layout, /tcher-live-svelte-start/, 'SvelteKit layout got the adapter marker');
          assert.match(layout, /TcherLiveRoot/, 'SvelteKit layout renders the adapter host');
          assert.doesNotMatch(appHtml, /tcher-live-start/, 'SvelteKit app.html must remain untouched');
        ...
        for (const r of result.results) {
          ...
          assert.match(body, /tcher-live-start/);
    ...
    it('live-inject --remove strips the script tag cleanly', () => {
      ...
      try {
        ...
        if (result.adapter === 'sveltekit') {
          const layout = readFileSync(join(tmp, 'src/routes/+layout.svelte'), 'utf-8');
          const appHtml = readFileSync(join(tmp, 'src/app.html'), 'utf-8');
          assert.doesNotMatch(layout, /TcherLiveRoot/);
          assert.doesNotMatch(layout, /tcher-live-svelte-start/);
          assert.doesNotMatch(appHtml, /tcher-live-start/);
          assert.equal(existsSync(join(tmp, 'src/lib/tcher/TcherLiveRoot.svelte')), false);
          return;
        }
        for (const r of result.results) {
          const body = readFileSync(join(tmp, r.file), 'utf-8');
          assert.doesNotMatch(body, /tcher-live-start/);
