...
describe('live-accept — style-element edge cases', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tcher-accept-test-')); });
  ...
  // separate closer, so it got stuck forever and missed data-tcher-variant
  // divs that came after.
  it('finds the accepted variant after a JSX self-closing <style /> block', () => {
    const html = `<body>
  <!-- tcher-variants-start SELFC -->
  <div data-tcher-variants="SELFC" data-tcher-variant-count="3" style="display: contents">
    <div data-tcher-variant="original">
      <p class="hook">original text</p>
    </div>
    <style data-tcher-css="SELFC" dangerouslySetInnerHTML={{ __html: '@scope ([data-tcher-variant="1"]) { .hook { color: red; } }' }} />
  ...
  <!-- tcher-variants-end SELFC -->
    ...
    // no carbonize block, no data-tcher-variant wrapper (it would serve no purpose).
    assert.ok(!after.includes('tcher-carbonize-start'), 'no carbonize block (self-closing style has no body)');
    assert.ok(!after.includes('tcher-variants-start'), 'variant markers removed');
  ...
  it('finds the accepted variant after a single-line <style>…</style> block', () => {
    const html = `<body>
  <!-- tcher-variants-start ONELINE -->
  <div data-tcher-variants="ONELINE" data-tcher-variant-count="3" style="display: contents">
    <div data-tcher-variant="original"><p class="hook">original</p></div>
    <style data-tcher-css="ONELINE">@scope ([data-tcher-variant="1"]) { .hook { color: red; } }</style>
    <div data-tcher-variant="1"><p class="hook">variant one</p></div>
    <div data-tcher-variant="2" style="display: none"><p class="hook">variant two</p></div>
    <div data-tcher-variant="3" style="display: none"><p class="hook">variant three</p></div>
  </div>
  <!-- tcher-variants-end ONELINE -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    const after = readFileSync(join(tmp, 'page.html'), 'utf-8');
    assert.ok(after.includes('data-tcher-variant="3"'), 'accepted wrapper for variant 3 present');
  ...
  it('finds the accepted variant after a multi-line <style>…</style> block (regression baseline)', () => {
    const html = `<body>
  <!-- tcher-variants-start MULTI -->
  <div data-tcher-variants="MULTI" data-tcher-variant-count="3" style="display: contents">
    <div data-tcher-variant="original"><p class="hook">original</p></div>
    <style data-tcher-css="MULTI">
      @scope ([data-tcher-variant="1"]) { .hook { color: red; } }
      @scope ([data-tcher-variant="2"]) { .hook { color: green; } }
    </style>
    <div data-tcher-variant="1"><p class="hook">variant one</p></div>
    <div data-tcher-variant="2" style="display: none"><p class="hook">variant two</p></div>
  </div>
  <!-- tcher-variants-end MULTI -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    const after = readFileSync(join(tmp, 'page.html'), 'utf-8');
    assert.ok(after.includes('data-tcher-variant="1"'), 'accepted wrapper for variant 1 present');
  ...
  it('carbonize does not double-wrap when the variants block uses JSX template literals on their own lines', () => {
    const tsx = `export default function App() {\n` +
      ...
      `    <main>\n` +
      `      <>\n` +
      `        {/* tcher-variants-start TPL */}\n` +
      `        <div data-tcher-variants="TPL" data-tcher-variant-count="2" style={{ display: 'contents' }}>\n` +
      `          <div data-tcher-variant="original"><p className="hook">orig</p></div>\n` +
      `          <style data-tcher-css="TPL">\n` +
      "            {`\n" +
      `              @scope ([data-tcher-variant="1"]) { .hook { color: red; } }\n` +
      `              @scope ([data-tcher-variant="2"]) { .hook { color: green; } }\n` +
      "            `}\n" +
      `          </style>\n` +
      `          <div data-tcher-variant="1"><p className="hook">variant one</p></div>\n` +
      `          <div data-tcher-variant="2" style={{ display: 'none' }}><p className="hook">variant two</p></div>\n` +
      `        </div>\n` +
      `        {/* tcher-variants-end TPL */}\n` +
      `      </>\n` +
      `    </main>\n` +
      `  );\n` +
      `}\n`;
    ...
    const carbonStyleMatch = after.match(/<style data-tcher-css="TPL">([\s\S]*?)<\/style>/);
    ...
    assert.ok(inner.includes('@scope ([data-tcher-variant="1"])'), 'variant-1 scope kept');
    assert.match(
      after,
      /\n          <div data-tcher-variant="1" style=\{\{ display: 'contents' \}\}>\n            <p className="hook">variant one<\/p>\n          <\/div>/,
  ...
  it('carbonize does not double-wrap when JSX template-literal punctuation hugs the CSS lines', () => {
    const tsx = `export default function App() {\n` +
      ...
      `    <main>\n` +
      `      <>\n` +
      `        {/* tcher-variants-start INLINE */}\n` +
      `        <div data-tcher-variants="INLINE" data-tcher-variant-count="2" style={{ display: 'contents' }}>\n` +
      `          <div data-tcher-variant="original"><p className="hook">orig</p></div>\n` +
      `          <style data-tcher-css="INLINE">\n` +
      "            {`@scope ([data-tcher-variant=\"1\"]) { .hook { color: red; } }\n" +
      "             @scope ([data-tcher-variant=\"2\"]) { .hook { color: green; } }`}\n" +
      `          </style>\n` +
      `          <div data-tcher-variant="1"><p className="hook">variant one</p></div>\n` +
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    const inner = after.match(/<style data-tcher-css="INLINE">([\s\S]*?)<\/style>/)[1];
    const openCount = (inner.match(/\{`/g) || []).length;
    const closeCount = (inner.match(/`\}/g) || []).length;
  ...
  it('carbonize preserves nested JSX indentation when the wrapper starts at column 0', () => {
    const tsx = `<div data-tcher-variants="ROOTIND" data-tcher-variant-count="2" style={{ display: 'contents' }}>
  {/* tcher-variants-start ROOTIND */}
  <div data-tcher-variant="original">
    <section className="hook">
      <span>original</span>
    </section>
  </div>
  <style data-tcher-css="ROOTIND">{\`@scope ([data-tcher-variant="1"]) { .hook { color: red; } }\`}</style>
  <div data-tcher-variant="1">
    <section className="hook">
...
`;
  ...
  return (
    ...
    // the outer wrapper, and no tcher markers/data attributes.
    assert.ok(!after.includes('data-tcher-variants'),
      `outer wrapper div must be fully removed; got:\n${after}`);
    assert.ok(!after.includes('data-tcher-variant'),
      `original-div wrapper must be fully removed; got:\n${after}`);
    assert.ok(!after.includes('tcher-variants-start'),
  ...
  return (
    <main>
      <div
        className="tcher-preview-shell"
${extraAttrs}
        data-tcher-variants="LONGOPEN"
        data-tcher-variant-count="2"
        style={{ display: 'contents' }}
      >
        {/* tcher-variants-start LONGOPEN */}
        {/* Original */}
        <div data-tcher-variant="original">
          <aside className="card">
            <h1>Original</h1>
        ...
        <div data-tcher-variant="1"><aside className="card"><h1>Variant</h1></aside></div>
        {/* tcher-variants-end LONGOPEN */}
      </div>
      <div className="next-card">After</div>
    </main>
  );
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    assert.doesNotMatch(after, /data-tcher-variants/);
    assert.doesNotMatch(after, /tcher-variants-start/);
  ...
  return (
    <main>
      <div data-tcher-variants="ACTIVE" data-tcher-variant-count="2" style={{ display: 'contents' }}>
        <div className="historical-marker-note">
          {/* tcher-variants-end OLD */}
        </div>
        {/* tcher-variants-start ACTIVE */}
        {/* Original */}
        <div data-tcher-variant="original">
          <aside className="card">
            <h1>Original</h1>
    ...
    const after = readFileSync(join(tmp, 'App.tsx'), 'utf-8');
    assert.doesNotMatch(after, /data-tcher-variants/);
    assert.doesNotMatch(after, /tcher-variants-start ACTIVE/);
    assert.doesNotMatch(after, /tcher-variants-end OLD/);
    assert.match(after, /<aside className="card">\s*<h1>Original<\/h1>\s*<\/aside>/m);
    assert.ok(after.includes('<div className="next-card">After</div>'));
  });
  ...
  return (
    <main>
      <div data-tcher-variants="INDENTACC" data-tcher-variant-count="3" style={{ display: "contents" }}>
        {/* tcher-variants-start INDENTACC */}
        {/* Original */}
        <div data-tcher-variant="original">
          <aside className="card">
            <h1 className="hero-title">Hero</h1>
        ...
        {/* Variants: insert below this line */}
        <div data-tcher-variant="1"><aside className="card variant-one"><h1 className="hero-title">Hero</h1></aside></div>
        {/* tcher-variants-end INDENTACC */}
      </div>
    </main>
  );
}`;
  ...
  it('discard restores the original element after a JSX self-closing <style />', () => {
    const html = `<body>
  <!-- tcher-variants-start DISC -->
  <div data-tcher-variants="DISC" data-tcher-variant-count="2" style="display: contents">
    <div data-tcher-variant="original"><p class="hook">ORIGINAL CONTENT</p></div>
    <style data-tcher-css="DISC" dangerouslySetInnerHTML={{ __html: '@scope ([data-tcher-variant="1"]) { .hook { color: red; } }' }} />
    <div data-tcher-variant="1"><p class="hook">variant one</p></div>
    <div data-tcher-variant="2" style="display: none"><p class="hook">variant two</p></div>
  </div>
  <!-- tcher-variants-end DISC -->
</body>`;
    writeFileSync(join(tmp, 'page.html'), html);
    ...
    assert.ok(!after.includes('tcher-variants-start'), 'wrapper markers gone');
...
describe('live-accept — insert sessions', () => {
  let tmp;
  beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), 'tcher-accept-insert-')); });
  afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

  const insertHtml = (id) => `<main>
  <section class="hero">Hero block</section>
  <!-- tcher-variants-start ${id} -->
  <div data-tcher-variants="${id}" data-tcher-mode="insert" data-tcher-variant-count="2" style="display: contents">
    <!-- Variants: insert below this line -->
    <style data-tcher-css="${id}">@scope ([data-tcher-variant="1"]) { .cta { color: red; } }</style>
    <div data-tcher-variant="1"><p class="cta">Variant one</p></div>
    <div data-tcher-variant="2" style="display: none"><p class="cta">Variant two</p></div>
  </div>
  <!-- tcher-variants-end ${id} -->
