...
Audit the code.`;
  ...
  test('should read skill with reference files', () => {
    const skillContent = `---
name: tcher
description: Tcher design skill
---

Tcher design instructions.`;
  ...
  test('should fall back to "tcher" when frontmatter has no name', () => {
    const skillDir = path.join(testRootDir, 'skill');
    ensureDir(skillDir);
    ...
    expect(skills).toHaveLength(1);
    expect(skills[0].name).toBe('tcher');
  });
...
describe('replacePlaceholders', () => {
  ...
  test('should exclude tcher from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'tcher', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('should exclude legacy teach-tcher from {{available_commands}}', () => {
    const result = replacePlaceholders('Commands: {{available_commands}}', 'claude-code', ['audit', 'teach-tcher', 'polish']);
    expect(result).toBe('Commands: /audit, /polish');
  });

  test('lists /tcher sub-commands for {{available_commands}} when no command names are passed', () => {
    // v3.0 single-skill architecture: with no command names, the list falls back
    // to the TCHER_SUB_COMMANDS sub-commands rendered as `/tcher <sub>`.
