...
describe('detectUrl — browser-only fixtures', () => {
  ...
  it('typography side-by-side: element-level flag cases get regular overlays', async () => {
    ...
    try {
      ...
      await page.evaluate(() => { window.__TCHER_CONFIG__ = { autoScan: false }; });
      await page.evaluate(browserScript);
      const result = await page.evaluate(() => {
        const groups = window.tcherScan();
        ...
        return {
          ...
          hasBanner: Boolean(document.querySelector('.tcher-banner')),
          overlays: document.querySelectorAll('.tcher-overlay:not(.tcher-banner)').length,
        };
      });
  ...
  it('browser API: visual contrast fallback resolves readable image backgrounds without overlays', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const before = document.querySelectorAll('.tcher-overlay, .tcher-label, .tcher-banner').length;
        const analyses = await window.tcherAnalyzeVisualContrast({ maxCandidates: 20, scrollOffscreen: true });
        const after = document.querySelectorAll('.tcher-overlay, .tcher-label, .tcher-banner').length;
        return {
          before,
          after,
  ...
  it('browser API: visual contrast scan decorates visible findings without scrolling by default', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        const syncScanResult = window.tcherScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const syncDetectResult = window.tcherDetect({
          visualContrast: true,
          serialize: true,
        });
        const groups = await window.tcherScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        return {
          ...
          overlays: document.querySelectorAll('.tcher-overlay:not(.tcher-banner)').length,
          labels: document.querySelectorAll('.tcher-label').length,
          analyses: window.tcherGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          scrollEvents,
          maxScrollY,
          finalScrollY: window.scrollY,
          syncScanIsArray: Array.isArray(syncScanResult),
          syncDetectIsArray: Array.isArray(syncDetectResult),
          hasAsyncApi: typeof window.tcherScanAsync === 'function' && typeof window.tcherDetectAsync === 'function',
        };
      });
      ...
      assert.equal(result.syncScanIsArray, true, `tcherScan should keep a synchronous Array return: ${JSON.stringify(result)}`);
      assert.equal(result.syncDetectIsArray, true, `tcherDetect should keep a synchronous Array return: ${JSON.stringify(result)}`);
      ...
      const refreshedOverlayResult = await page.evaluate(async () => {
        ...
        const initialGroups = window.tcherScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        while (
          Date.now() < deadline &&
          !/low contrast/i.test(target?._tcherOverlay?.textContent || '')
        ) {
          const nextButton = target?._tcherOverlay?.querySelector('button:last-of-type');
          if (nextButton) nextButton.click();
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const labelVariants = [];
        const overlay = target?._tcherOverlay;
        for (let i = 0; i < 3; i++) {
        ...
        return {
          ...
          labelText: target?._tcherOverlay?.textContent || '',
          labelVariants,
          overlayConnected: Boolean(target?._tcherOverlay?.isConnected),
        };
      });
      ...
      const lazyResult = await page.evaluate(async () => {
        ...
        return {
          ...
          labels: document.querySelectorAll('.tcher-label').length,
          analyses: window.tcherGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          targetHasOverlay: Boolean(target?._tcherOverlay),
      ...
      const staleOverlayResult = await page.evaluate(async () => {
        ...
        window.scrollTo(0, 0);
        await new Promise(resolve => setTimeout(resolve, 50));
        await window.tcherScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const staleCleared = !target?._tcherOverlay;
        target?.scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'instant' });
        await new Promise(resolve => setTimeout(resolve, 250));
        return {
          staleCleared,
          targetHasOverlay: Boolean(target?._tcherOverlay),
          targetOverlayConnected: Boolean(target?._tcherOverlay?.isConnected),
          overlays: document.querySelectorAll('.tcher-overlay:not(.tcher-banner)').length,
          analyses: window.tcherGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
        };
      });
  ...
  it('extension mode remove cancels pending lazy visual contrast work', async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        window.__tcherMessages = [];
        window.addEventListener('message', event => {
          if (event.source !== window || !event.data?.source?.startsWith('tcher-')) return;
          window.__tcherMessages.push(event.data);
        });
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(async () => {
        ...
        while (
          ...
          !window.tcherGetLastVisualContrastAnalyses()
            .some(item => item.status === 'unresolved' && item.reason === 'text outside viewport')
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const unresolvedBeforeRemove = window.tcherGetLastVisualContrastAnalyses()
        ...
        const resultsAfterRemove = window.__tcherMessages
          .filter(message => message.source === 'tcher-results').length;
        return {
          unresolvedBeforeRemove,
          overlayCount: document.querySelectorAll('.tcher-overlay').length,
          targetHasOverlay: Boolean(target?._tcherOverlay),
          resultsAfterRemove,
        };
      });
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          while (
            Date.now() < deadline &&
            !window.__tcherMessages.some(message => message.source === 'tcher-error')
          ) {
            await new Promise(resolve => setTimeout(resolve, 25));
          }
          return {
            ready: window.__tcherMessages.some(message => message.source === 'tcher-ready'),
            results: window.__tcherMessages.some(message => message.source === 'tcher-results'),
            errors: window.__tcherMessages
              .filter(message => message.source === 'tcher-error')
  ...
  it('extension mode echoes scan ids on result messages', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        while (
          Date.now() < deadline &&
          !window.__tcherMessages.some(message =>
            message.source === 'tcher-results' &&
            message.scanId === 'scan-2'
          )
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const resultMessage = window.__tcherMessages.find(message =>
          message.source === 'tcher-results' &&
          message.scanId === 'scan-2'
        );
        return {
  ...
  it('browser API: tcherDetect is pure, tcherScan decorates', async () => {
    const puppeteer = await import('puppeteer');
    const browser = await puppeteer.default.launch({
      headless: true,
      args: process.env.CI ? ['--no-sandbox', '--disable-setuid-sandbox'] : [],
    ...
    try {
      ...
      const pure = await page.evaluate(() => {
        const before = document.querySelectorAll('.tcher-overlay, .tcher-label, .tcher-banner').length;
        const findings = window.tcherDetect({ decorate: false, serialize: true });
        const after = document.querySelectorAll('.tcher-overlay, .tcher-label, .tcher-banner').length;
      ...
      const decorated = await page.evaluate(() => {
        const groups = window.tcherScan();
        const overlays = document.querySelectorAll('.tcher-overlay, .tcher-label, .tcher-banner').length;
  ...
  it('browser API: async scan and detect reject instead of throwing synchronously', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          const scan = await window.tcherScanAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          const detect = await window.tcherDetectAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          return { scan, detect };
