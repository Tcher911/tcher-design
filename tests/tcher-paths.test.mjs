/**
 * Tests for project-local Tcher path resolution.
 * Run with: node --test tests/tcher-paths.test.mjs
 */
...
describe('tcher project paths', () => {
  let tmp;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'tcher-paths-'));
  });
  ...
  it('resolves the generated design sidecar under .tcher', () => {
    assert.equal(getDesignSidecarPath(tmp), join(tmp, '.tcher', 'design.json'));
  ...
  it('uses .tcher/live/config.json as the default live config path', () => {
  ...
  it('falls back to legacy scripts/config.json when no new live config exists', () => {
    const scriptsDir = join(tmp, 'skills', 'tcher', 'scripts');
  ...
  it('lets TCHER_LIVE_CONFIG override both new and legacy config locations', () => {
    ...
    assert.equal(
      resolveLiveConfigPath({ cwd: tmp, scriptsDir: join(tmp, 'scripts'), env: { TCHER_LIVE_CONFIG: 'custom-live-config.json' } }),
      override,
    );
  });

  it('places live server, session, and annotation state under .tcher/live', () => {
    assert.equal(getLiveServerPath(tmp), join(tmp, '.tcher', 'live', 'server.json'));
    assert.equal(getLiveSessionsDir(tmp), join(tmp, '.tcher', 'live', 'sessions'));
    assert.equal(getLiveAnnotationsDir(tmp), join(tmp, '.tcher', 'live', 'annotations'));
  });
